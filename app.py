...
ARQ_TEMP = ARQDADOS + '.tmp' # Segurança: Arquivo temporário
ARQ_CHECKPOINT = 'checkpoint.json' # Segurança: Controle de progresso
...
WL_NUTRI_CLINICA = [normalize(x) for x in ["NUTRICAO ENTERAL", "FORMULA INFANTIL", "SUPLEMENTO ALIMENTAR", "DIETA ENTERAL", "DIETA PARENTERAL", "NUTRICAO CLINICA", "ENTERAL", "PARENTERA"]]
...
def processar_licitacao(lic, session, forcado=False):
    ...
    except Exception as e: return ('ERRO', {'msg': f"Erro interno em {id_ref}: {str(e)}"}, 0, 0)
...
def buscar_periodo(session, banco, d_ini, d_fim):
    ...
    for i in range(delta.days + 1):
        ...
        # SEGURANÇA: Pula dias já concluídos no checkpoint
        if checkpoint and dia < checkpoint['dia']: continue

        print(f"\n📅 DATA: {dia}")
        url = 'https://pncp.gov.br/api/consulta/v1/contratacoes/publicacao'
        
        # SEGURANÇA: Retoma da página correta
        ...
        while True:
            try:
                ...
                # SEGURANÇA: Salva checkpoint após cada página processada
...
if __name__ == '__main__':
    ...
    try:
        ...
        # SEGURANÇA: Gravação Atômica (Safe Write)
        print("\n💾 Salvando banco de dados com segurança...")
        with gzip.open(ARQ_TEMP, 'wt', encoding='utf-8') as f:
            json.dump(list(banco.values()), f, ensure_ascii=False)
        
        if os.path.exists(ARQ_TEMP):
            os.replace(ARQ_TEMP, ARQDADOS) # Substituição segura
            if os.path.exists(ARQ_CHECKPOINT): os.remove(ARQ_CHECKPOINT) # Limpa checkpoint se terminou tudo
