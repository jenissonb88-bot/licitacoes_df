...
MAXWORKERS = 8 # Aumentado para processar mais rápido as páginas extras
...
def processar_licitacao(lic_resumo, session):
    try:
        ...
        # Sincronização Ativa: Baixa resultados sempre
...
def buscar_dia_completo(session, data_obj, banco):
    ...
    while True:
        ...
        try:
            ...
            total_paginas = payload.get('totalPaginas', 1) # USA O TOTAL DE PÁGINAS DO PNCP
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=MAXWORKERS) as exe:
                ...
                for futuro in concurrent.futures.as_completed(futuros):
                    ...
                    if res:
                        banco[res['id']] = res
                        total_capturados += 1
                        print(f"   DISK-WRITE: {res['uf']} - {res['edit']} | Vencedores: {sum(1 for i in res['itens'] if 'res_forn' in i)}")
            
            if pag >= total_paginas: break # SÓ PARA SE CHEGAR NA ÚLTIMA PÁGINA REAL
            pag += 1
        except: break
...
if __name__ == '__main__':
    ...
    try:
        ...
        total = 0
        for dia in datas: total += buscar_dia_completo(session, dia, banco)
        if total > 0:
