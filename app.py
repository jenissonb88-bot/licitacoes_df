...
MAXWORKERS = 8 # Processamento paralelo para compensar as buscas individuais
...
def buscar_resultado_item(session, cnpj, ano, seq, seq_item):
    """Busca o vencedor de um item específico no PNCP"""
...
def processar_licitacao(lic_resumo, session):
    try:
        ...
        # 1. Busca todos os itens do pregão
        ...
        for item in itens_raw:
            try:
                ...
                if res_data:
                    item_obj['res_forn'] = res_data.get('nomeRazaoSocialFornecedor')
