...
def validar_regiao(lic):
    ...
    # NORDESTE: especiais OU medicamentos
    if uf in UFS_NE:
        return (any(t in obj_norm for t in ['MEDICAMENTO','FARMACIA','INSUMO FARMACEUTICO']) or 
                any(t in obj_norm for t in ['MATERIAL MEDICO','DIETA ENTERAL','FORMULA','LU VAS','ALCOOL']))
    
    # DEMAIS UFs: só medicamentos
    if uf in UFS_OUTROS:
...
def limpar():
    ...
    bancofinal = []
    mapasit = {1: 'EM ANDAMENTO', 2: 'HOMOLOGADO', 3: 'ANULADO', 4: 'REVOGADO', 5: 'FRACASSADO', 6: 'DESERTO'}
    ...
    for lic in dados:
        ...
        # Processa itens com match obrigatório
        itensproc = {}
        has_match = False
        ...
        # Exige pelo menos 1 item com match pharma
        ...
        # Remove raws
        for k in ['itensraw', 'resultadosraw', 'valorGlobalApi']:
            lic.pop(k, None)
        bancofinal.append(lic)
